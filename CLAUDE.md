...
- 8방향 드래그 연결, 경로 내 재선택 금지, 우클릭·ESC 취소
  ...
  - **놓으면 턴을 쓴다** (보드 밖에서 놓아도 동일, 사용자 결정): 2개 이상이면 보석 사용(제거·낙하·보충), **1개면 효과 없이 턴만 소모**
  - **취소(턴 유지)**: 보드 양옆 바깥의 **보이지 않는 취소 영역**에서 놓기, 드래그 중 우클릭·ESC
    - 취소 영역은 그리지 않는다(사용자 결정). 드래그 중 포인터가 들어가면 **화면이 살짝 붉어져** 지금 놓으면 취소됨을 알린다
    - 영역: 보드 카드 가장자리 24px 바깥부터 화면 끝까지, 위아래 전체
  - 마우스 판정은 칸 중심의 **80% 원** 안 — 대각선으로 그을 때 옆 칸이 잘못 잡히지 않게
  - 연결선은 **강조색(금색) 한 가지**, 가늘게. 선택 표시는 칸 외곽선·확대가 맡는다
- 행동력(기획서 5.4): 기본 6, 보석 1개 = 1 → 경로 최대 길이. **남은 행동력은 이월하지 않는다**(사용자 결정). 다음 턴 보너스(균형 보석·스킬)는 다음 턴에만 더해진다. 기본값 변경은 다음 턴부터
- 한 턴에 연결은 한 번. 쓰고 나면 다음 턴까지 보드가 잠긴다(어둡게 표시)
- 제거·낙하·보충(기획서 5.1): 사용한 보석을 없애고 같은 열의 남은 보석을 아래로 내린 뒤 빈 윗칸을 새 보석으로 채운다. 보충 순서는 열 0부터·각 열 아래 빈칸부터이고, 보충용 난수도 보드 시드에서 만든다(같은 시드 + 같은 조작 = 같은 결과)
- 보석 효과(기획서 5.2·5.5): **연결 순서대로** 하나씩 계산한다. `GemEffectResolver`는 결과표(`EffectSummary`)만 돌려주고 대상에 적용하지 않는다 — 전투(`BattleManager`)·퍼즐 테스트가 적용한다
  - 물리·마법·회복 1개 = 메인 스탯 × 보석 계수(`GemData.effectCoefficient`) × 배율. 보석마다 반올림(0.5 올림), 최소 1
  - 혼돈 1개 = 대상 적 행동 +1, **한 번 연결에 최대 +2** / 균형 1개 = 다음 턴 행동력 +1, **다음 턴 보너스 최대 +3** (`PuzzleConfig`)
  - 균형 보너스는 `PuzzleManager`가 확정 즉시 `ActionPoints`에 반영한다
  - 배율 자리: 9일차 동일 종류 연속 +20%, 10일차 피버 +50%가 보석 순번별 배율로 들어간다
- 공격 대상(기획서 6.3, `Gameplay/Battle/TargetSelection`): 처음엔 화면 왼쪽(순번 0) 적. **플레이어가 클릭해 살아 있는 적으로 바꾼다.** 현재 대상이 쓰러지면 **살아 있는 적 중 무작위**(사용자 결정, 시드 `System.Random`). 남은 적이 없으면 대상 없음
  - 한 번 연결 도중 대상이 쓰러지면 **남은 보석 효과는 새 대상에게** 간다(순서대로 처리, 5.5). 쓰러뜨린 공격의 남는 피해는 넘기지 않는다
  - 혼돈 지연도 현재 대상에게 준다
- 동일 종류 연속(기획서 5.5·5.6, `ChainBonus`): 같은 종류 **일반** 보석이 경로에서 바로 이어진 구간마다 2번째부터 **+20% 누적** (100% / 120% / 140% …, **상한 없음** — 행동력이 길이를 막는다)
  - 연속 배율은 피해·회복에만 곱한다. 혼돈·균형 양은 1씩이라 배율을 받지 않고 4연속 보너스로 보상한다
  - 다른 종류나 **특수 보석이 끼면 끊긴다**. 특수 보석은 연속 배율을 받지 않는다(외부 배율 = 10일차 피버는 받는다)
  - **4연속 이상** 구간이 끝나는 자리에 연속 강화 1건: 물리·마법 **강공격**(구간 피해 합 × 50% 추가), 회복 **추가 회복**(구간 회복 합 × 50%), 혼돈 **지연 강화**(+1, 혼돈 1회 상한 +2와 별개), 균형 **다음 턴 행동력 +1**(보너스 상한 +3은 지킴)
  - **5연속 이상**이면 경로 **마지막 칸**의 보석을 없애지 않고 그 구간 종류의 **특수 보석**으로 바꾼 뒤 낙하시킨다. **한 번 연결에 1개**, 조건을 넘긴 구간이 여럿이면 마지막 구간 종류
- 특수 보석(기획서 5.6.1): `Board`가 칸마다 특수 여부를 들고, 낙하 때 함께 옮긴다. 보충 보석은 늘 일반. 일반 보석처럼 이어서 쓰면 발동
  - 물리 = 한 적에게 물리 × 3 / 마법 = **모든 적**에게 마법 × 1.5 / 회복 = 회복력 × 3 (상태 효과 제거는 상태 효과 도입 때) / 혼돈 = **모든 적** 행동 +1(혼돈 상한과 별개) / 균형 = 다음 턴 행동력 +2(보너스 상한은 지킴)
  - 전투 종료 시 제거(기획서)는 전투(11일차~)에서 연결한다. 퍼즐 테스트에서는 새 보드를 만들면 사라진다
- 연결 중 실시간 표시(사용자 요청): 연결한 칸 오른쪽 아래에 그 칸의 연속 배율(×1.2 …, 4연속째는 청록, 특수 보석은 "특수"), 보드 왼쪽 아래 **연결 효과** 칸에 마지막 보석의 배율·"물리 3연속"·다음 안내·합계. 계산은 `PuzzleManager.PreviewEffects`(확정과 같은 계산기)
...
- 퍼즐 HUD 배치: 행동력 표시는 **보드 상단 중앙**(보드 카드 윗변에 걸침, 사용자 결정), 취소 영역은 **보드 양옆 바깥, 보이지 않음**(들어가면 `CancelOverlay`가 화면 가장자리를 옅게 붉게 물들임: 가장자리 안쪽 5% 구간만, 강도 25%. 사용자 요청으로 연하고 좁게 조정).
- Dev_PuzzleTest 배치: 상단 카드 시드 / **허수아비 3마리(가운데, 왼쪽부터 순번 0·1·2)** / 연결 기록, 보드 왼쪽 위 메인 캐릭터·**왼쪽 아래 연결 효과**(9일차), **보드 오른쪽 보석 분포 + 연속 보너스 안내**(허수아비와 자리 바꿈, 사용자 요청). 양옆 칸은 상단 카드 아래부터 보드 카드 아랫변까지.
- Unity 오브젝트에는 `??` / `?.` 연산자를 쓰지 않는다. Unity의 null 판정을 거치지 않아 없는 컴포넌트를 있다고 판단할 수 있다.
...
- [x] **7일차** 행동력·턴당 1회·보이지 않는 취소 영역 + **제거·낙하·보충(9일차 항목을 앞당김, 사용자 요청)** — Build All 완료, Test Runner 56/56 (Play 조작감은 기록으로 미확인)
- [x] **8일차** 기본 보석 효과 (물리·마법·회복·혼돈·균형) + **공격 대상 선택·처치 시 무작위 재지정(13일차 항목 일부 앞당김, 사용자 요청)** — Build All 완료, Test Runner 76/76, Dev_PuzzleTest Play 오류 없음
- [x] **9일차** 동일 종류 연속 +20%·4연속 연속 강화·5연속 특수 보석 + **연결 중 칸별 배율·연결 효과 실시간 표시(사용자 요청)** (제거·낙하·보충은 7일차에 완료) — Build All 완료, Test Runner 97/97, Dev_PuzzleTest Play 오류 없음
- [ ] 10일차 피버 + 퍼즐 검증
...
- **임시 적용 중** (밸런스·아트 단계에서 재검토):
  - 9일차 제안값(`PuzzleConfig`): 연속 +20%·상한 없음, 강공격·추가 회복 +50%, 지연 강화 +1, 균형 4연속 +1, 특수 보석 물리 ×3·마법 ×1.5(모든 적)·회복 ×3·혼돈 +1(모든 적)·균형 +2
  - 보석 생성 확률 5종 균등(가중치 20) / 혼돈 보석 색은 검정 대신 짙은 보라
  - 보석 효과 계수 1.0 / 혼돈 1개 +1·최대 +2 / 균형 1개 +1·최대 +3 / 보석마다 반올림·최소 1 (8일차)
  - 퍼즐 테스트용 메인 임시 스탯: 물리 10·마법 10·회복력 8, HP 100 (11일차 캐릭터 데이터 전까지)
