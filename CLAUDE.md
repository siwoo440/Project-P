...
- 타이틀 UI 조립 도구: 메뉴 `Project P > Build Title UI`. 다시 실행하면 `[TitleUI]`를 지우고 새로 만든다. 씬에서 직접 다듬은 배치는 사라진다.
...
- [x] **3일차** 00_Boot 및 전역 서비스 — Boot→Title 전환·settings.json 생성 확인 (Dev 씬 복귀·손상 파일·초기화 실패 처리는 미확인)
